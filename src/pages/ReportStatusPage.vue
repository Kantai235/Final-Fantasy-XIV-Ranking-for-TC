...
const 常見問題分類 = Object.freeze([
  ...
  },
  {
    ...
    label: "排行榜",
    title: "排名、公開與角色身分",
    summary: "排行榜會依不同職業與紀錄條件建立排名，也會尊重 FFLogs 公開狀態與繁中服角色身分。",
    items: [
      {
        question: "為什麼兩筆 rDPS 幾乎相同，PR 卻可能差很多？",
        answer: [
          "本站的職業 Rank 與同職分位都是以「同副本、同職業」分開比較，不是把同一職能或所有職業放在同一個 PR 母體。rDPS 會調整團隊增益貢獻，但不會把不同職業的輸出能力校正成相同尺度；因此相同的 rDPS 在不同職業分布中，可能落在完全不同的位置。",
          "以 2026-08-09 收到的 M5S 回報為例：絕槍戰士紀錄 RrzFndmX3gt6K4wV fight 2 的 rDPS 是 19,480.95，暗黑騎士紀錄 J8TqwdVKBG4PCpzR fight 19 是 19,480.00。兩筆若暫時不分職業、只按 rDPS 排序，分別是第 1267、1268 名，幾乎相鄰；但絕槍戰士當時有更多紀錄高於這個數值，暗黑騎士則較少，所以各自的同職 PR 會相差很多。",
          "PR 表示的是這筆紀錄相對於同職業樣本的位置，不適合直接拿來比較不同職業的絕對輸出。若想觀察不同職業的輸出差異，請改看全服統計的「全職業輸出比較」，其中會列出各職業的中位數、四分位數與最大值。",
        ],
      },
      {
        question: "職業 Rank、同職分位和同職能 PR 有什麼差別？",
        answer: [
          "職業 Rank 會在同副本、同職業的有效版本紀錄中，以「角色名稱 + 伺服器 + 職業」去重，每位角色只保留該職業的最佳紀錄；畫面下方的前 N%／PR，則由職業 Rank 與該職業通關人數換算。",
          "同職分位同樣只和同職業比較，但會納入所有非過版、rDPS 完整且 Active 達 50% 的有效紀錄，不會先把每位角色縮成一筆最佳成績。因此它和職業 Rank 的樣本數、排名與 PR 可能略有不同。以上述回報當時的快照為例，職業 Rank 換算約為絕槍戰士 PR 57、暗黑騎士 PR 91；同職分位則約為絕槍戰士 PR 56、暗黑騎士 PR 92。",
          "本站目前沒有計算「同職能 PR」；無論是職業 Rank 或同職分位，都是同職業比較。若把坦克、治療或輸出職能內的不同職業直接混算，結果會同時受到職業平衡影響，讓某些基礎輸出較低的職業很難取得高 PR，反而無法公平呈現玩家在自己職業中的相對表現。",
        ],
      },
      {
  ...
  <section class="常見問題頁" aria-live="polite">
    <section class="常見問題前言" aria-label="常見問題摘要">
      <div class="常見問題前言主文">
        ...
        <h2>常見問題</h2>
        <p>把排行榜最常被問到的收錄、更新、排名、過版、GCD 與角色身分問題整理在同一頁；FFLogs 相關疑問可以先用檢查工具比對目前公開索引。</p>
      </div>
