...
const 使用者成就手冊 = computed(() => {
  ...
  // 手冊會在零式分頁完整比較四階全站統計，但玩家徽章與總進度仍維持「同量級
  // 只取得最高階一枚」。`目前適用` 專門指出未取得玩家此刻應追蹤的階級，讓
  ...
  return 個人成績成就目錄.map((定義) => {
    ...
    return {
      ...
      // `目前適用` 只描述四階互斥進度的當前目標；炒股仔等額外成就會一直
      // 留在目錄中，但不應被 Vue 誤標成限時階段目標。
      目前適用: 定義.手冊項目類型 === "stage" && 目前適用成就Id.has(定義.id),
      手冊分類: 定義.手冊分類,
      手冊群組: 定義.手冊群組,
      手冊群組名稱: 定義.手冊群組名稱,
      手冊進度群組: 定義.手冊進度群組,
