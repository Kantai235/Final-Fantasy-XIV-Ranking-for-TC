...
// 空值只作為「尚未手動選擇」的內部狀態；介面會依薄索引的 starts_at_iso，永遠顯示
// 瀏覽當下已開放的最新實際版本。分享網址也只在開啟版本紀錄且使用者特別選擇歷史
// 版本時帶 gameVersion，避免把會隨時間前進的「目前版本」固化成網址條件。
const 排行榜遊戲版本 = ref("");
...
function 比較個人成績版本(左側版本, 右側版本) {
  ...
  if (Number.isFinite(左側數值) && Number.isFinite(右側數值) && 左側數值 !== 右側數值) {
    // FFXIV 的 7.05、7.1、7.15 等版本號可直接以數值比較，避免字串排序讓這些
    // 不等位數的小版本順序難以維護。選單以較新版本在前，方便查看目前與近期的成績環境。
    return 右側數值 - 左側數值;
  }
...
const 使用者版本選項 = computed(() => {
  const 版本集合 = new Set(
    使用者完整副本成績.value
      ...
      .map((成績) => 取得個人成績紀錄版本(成績))
      .filter(Boolean),
  );

  return Array.from(版本集合)
    .sort(比較個人成績版本)
    .map((版本) => ({ value: 版本, label: 版本 }));
});
...
const 使用者簡表版本副本成績 = computed(() => {
  ...
  // 也保留一般個人成績頁與報告彈窗的完整追溯能力。職業條件與一般成績單共用，
  // 讓使用者切換模式後仍留在相同檢視範圍；空條件就是預設的全部職業。
  return 取得使用者副本成績(
    使用者資料.value,
    使用者伺服器篩選.value,
    (成績) => (
      成績符合個人成績簡表版本(成績, 使用者簡表版本.value)
      && 符合使用者職業篩選(成績)
    ),
    使用者代表成績是否較佳,
  );
...
const 使用者簡表群組 = computed(() => {
  // 建立器只負責把目前版本與職業範圍內的成績整理成固定副本清單；沒有命中的
  // 目標副本仍會保留為「尚未收錄」，才能清楚回答該職能／職業在哪些副本有紀錄。
  return 建立個人成績簡表群組(
    副本清單.value,
    使用者簡表版本副本成績.value,
    使用者簡表版本.value,
...
// 單一職業玩家沒有可切換的範圍，隱藏選單可減少簡表查詢列的干擾；多職玩家
// 才顯示既有的職能／職業兩階選單，初始空篩選則維持「全部職業」。
const 使用者有多個職業 = computed(() => 使用者可用職業列表.value.length > 1);
...
function 成績屬於使用者版本快照(成績, 目標版本) {
  const 紀錄版本 = 取得個人成績紀錄版本(成績);
  if (!紀錄版本) {
    // 歷史資料若缺少可判定版本，無法安全放入舊版本快照；但最新版本代表完整
    // 成績單，因此仍保留這類紀錄，避免使用者開啟版本欄位後誤以為資料遺失。
    return 目標版本 === 使用者版本選項.value[0]?.value;
  }

  // 比較個人成績版本以較新版本排在前面；結果大於或等於零，表示這筆紀錄與
  // 目標版本相同或更早。例如選擇 7.1 時，7.0、7.05、7.1 都會納入快照。
  return 比較個人成績版本(紀錄版本, 目標版本) >= 0;
}
...
function 符合使用者版本篩選(成績) {
  ...
  const 目標版本 = 使用者版本篩選.value || 使用者版本選項.value[0]?.value || "";
  return Boolean(目標版本) && 成績屬於使用者版本快照(成績, 目標版本);
}

function 符合使用者個人成績篩選(成績) {
  return 符合使用者職業篩選(成績) && 符合使用者版本篩選(成績);
}

const 使用者副本成績 = computed(() => {
  return 取得使用者副本成績(使用者資料.value, 使用者伺服器篩選.value, 符合使用者個人成績篩選, 使用者代表成績是否較佳);
});
...
const 使用者成就手冊 = computed(() => {
  ...
  const 階段狀態索引 = 建立成就手冊階段狀態索引(
    已獲得成就Id,
    成就手冊目前時間戳記.value,
  );

  // 手冊會在零式分頁完整比較五階全站統計，但玩家徽章與總進度仍維持「同量級
  // 只取得最高階一枚」。工具層會依截止時間與互斥結果提供每階段的完整狀態，
  // Vue 不必從名稱或日期重做規則。舊版 users/index.json 沒有統計時仍顯示完整
  // 目錄，僅將人數標成待更新，避免 CDN 短暫版本差讓分頁或清單消失。
  return 個人成績成就目錄.map((定義) => {
    const 統計 = 索引統計.get(定義.id);
    const 獲得人數 = Number(統計?.holder_count);
    const 獲得占比 = Number(統計?.holder_percentage);
    const 手冊階段狀態 = 定義.手冊項目類型 === "stage"
      ? 階段狀態索引.get(定義.id) || null
      : null;
    return {
      id: 定義.id,
      // 文案與徽章共用程式內定義，避免資料快照或 CDN 尚未更新時仍顯示舊名稱。
      // 獲得人數與占比仍讀取靜態索引，調整文案不必重建玩家資料。
      名稱: 定義.名稱,
      說明: 定義.說明,
      // 局部刪除線只屬於目前前端的呈現中繼資料；舊版與新版 users/index.json
      // 都維持純文字 description，避免為單一樣式擴張跨 repo 的靜態資料契約。
      說明片段: 定義.說明片段,
      ...
      手冊階段狀態,
      // 保留既有布林欄位供相容檢查使用；完整顯示改讀工具層提供的語意狀態。
      目前適用: 手冊階段狀態?.id === "current",
      手冊分類: 定義.手冊分類,
      手冊群組: 定義.手冊群組,
      手冊群組名稱: 定義.手冊群組名稱,
      手冊進度群組: 定義.手冊進度群組,
...
function 套用說明提示顯示(啟用, { 寫入偏好 = true } = {}) {
  ...
  // 說明提示也會出現在 Teleport 彈窗中；寫到根節點可讓一般頁面與彈窗同步套用同一個偏好。
...
watch([
  ...
  // 篩選結果更換後不沿用舊副本的排序與同場列狀態，避免使用者誤把不同角色、
  // 版本或職業範圍視為同一檢視；簡表沒有歷史列，切入時也一併關閉。
  使用者歷史排序設定.value = {};
  顯示使用者同場職能職業.value = false;
});
...
watch([顯示版本紀錄, 使用者版本選項], () => {
  ...
  const 最新可用版本 = 使用者版本選項.value[0]?.value || "";
  const 已選版本仍可用 = 使用者版本選項.value.some((版本) => 版本.value === 使用者版本篩選.value);
  if (!已選版本仍可用) {
    // 不提供「全部版本」選項：較新版本已包含先前版本的歷史資料，因此最新版本
    // 就是完整成績單的預設檢視。
