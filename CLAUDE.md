...
3. `ultimate_bahamut` 的多階段敵方生命池語意不能套用此倍率檢核，必須寫入 `not_applicable`，避免把正常歷史戰鬥誤判。其他無法取得最大 HP 的 fight 只能寫 `unverifiable`，除非同時有 `Attack` 標記，或已超過歷史傷害預篩上緣，才可隱藏為 `suspected`。
4. 這是可撤除的資料品質防護。GitHub Actions 用 `FFLOGS_FIGHT_INTEGRITY_REPORT_LIMIT` 小批量逐輪補查，`FFLOGS_FIGHT_INTEGRITY_ENABLED=false` 停止新增檢核；日後 Log 工具修正後可停止 workflow 步驟與回補腳本。已標記的歷史 fight 與判定證據必須繼續保留，不得硬刪；判為異常的 v8 fight 在 v9 重判完成前維持隱藏，若 v9 明確判為 `valid`／`not_applicable` 才可恢復公開。
5. `scripts/fight_integrity_cache.py` 只可保存重新套用規則所需的「彙總敵方承傷、生命池、目標數」最小測量資料，或可重現的無法量測原因，預設置於 `.gitignore` 排除的 `data/local-cache/fight-integrity/measurements.json`；不得將依目標表格、raw events、玩家資料、OAuth 資訊或完整 API payload 寫入。每次成功量測或確認無法量測後須原子寫入，讓中斷不浪費已使用的 FFLogs 額度；既有 `data_integrity.metrics` 與無法量測結果必須可直接植入快取，不得為了補快取重讀 API。同一 report/fight 的來源指紋未變時，`--force` 也必須優先以快取離線重判，只有 `--refresh-cache` 或指紋變更才可再次取用 API。GitHub Actions 以 Actions cache 接續這份不進 Git 的快取。
6. `config/fight_integrity_baselines.json` 與 `scripts/fight_integrity_baselines.py` 是可獨立移除的歷史預篩層。它只使用 2026-07-28 18:00 前、`fight_hash` 去重後、完整繁中隊伍的 `players[].total_damage` P99；此欄位會受 Limit Break、Pet 與多目標傷害影響，絕不是 Boss HP。舊副本完整隊伍低於 P99×1.05 時可直接寫入本地 `valid`，節省生命池 API；高於上緣時必須繼續查快取或 FFLogs 生命池，基準本身不得寫入 `excluded`。極澤蓮尼亞、幻朱雀與樣本不足的副本不放入基準設定，仍完全走生命池／Attack 路徑。
7. `config/fight_integrity_known_enemy_hp.json` 與 `scripts/fight_integrity_known_capacity.py` 是比歷史預篩更優先、可獨立撤除的固定規則。極澤蓮尼亞要求完整繁中隊伍的 `players[].total_damage` 總和落在 `92,086,132–92,086,332`；幻朱雀依站務暫時性收錄決定，要求落在 `71,280,000–72,720,000`（72m ±1%）。兩者在玩家合計落於範圍內且沒有 `Attack` 標記時可直接寫入 `valid`，高於上限時可直接標記異常；低於下限只代表玩家傷害下限不足，因為 Limit Break 不會歸屬 `players[]`，必須改以既有快取或 FFLogs Target Damage 比較相同範圍。當 `players[]` 未完整涵蓋隊伍時也使用同一敵方承傷路徑，不可因玩家列缺人而跳過固定總傷害規則。幻朱雀的固定生命池 `127,613,543` 仍保留作為可追溯量測資訊，但固定範圍優先於下限倍率。絕伊甸、絕巴哈姆特、M1、M3、M4 的完整隊伍總傷害高於 `151,500,000`、`13,230,230`、`75,870,000`、`96,523,000`、`114,526,000` 時，一律標示為 `suspected` 並隱藏。這些是只有超標才成立的獨立硬上限，不能將未超標資料寫成 `valid`；它們在多階段副本的生命池 `not_applicable` 之前執行，避免 243m 類異常藉此例外重新出現在公開資料。切點後缺少結果、`unverifiable` 或任何非 `valid`／`not_applicable` 狀態均採 fail-closed，不能出現在排行榜、個人成績、隊伍榜或近期動態。`backfill_fight_integrity.py --offline-only` 不建立 OAuth、不呼叫 API，只用既有快取、固定規則和歷史基準；不能離線確認時保留 `unverifiable` 並隱藏。
8. `data_integrity.calculation_version=8` 且狀態為 `valid`／`not_applicable`、`hidden_from_public=false` 的既有結果列入公開相容清單，規則升至 v9 時必須繼續公開，也不得占用日常回補額度；v8 的 `excluded`／`suspected`／`unverifiable` 才逐批排入 v9 重判。缺少結果、早於 v8 的版本或其他未知舊版仍一律 fail-closed。人工明確使用 `--force` 時可全量稽核；一般回補則會先把失敗結果已保存的最小量測資料植入不進 Git 的快取後離線重判。`.github/workflows/update_rankings.yml` 必須在推送前比對該輪起始的完整性規則檔指紋；若遠端已變更，舊 runner 必須停止而非 rebase 後回寫過期資料。
9. 極澤蓮尼亞與幻朱雀的 `required_enemy_damage_min`／`required_enemy_damage_max` 是完整隊伍角色列不完整，或完整玩家合計低於固定下限時的總傷害替代證據：只能使用既有快取或 FFLogs Target Damage 的敵方承傷，分別比對 `92,086,132–92,086,332` 與 `71,280,000–72,720,000` 範圍。這是副本個別明示的例外，不得推廣到其他多目標副本；未設定此範圍者仍只能使用角色全隊傷害規則或生命池量測。
10. 絕伊甸的 `maximum_enemy_damage=151,500,000` 與既有 `maximum_full_party_damage` 是互補的單向上限：玩家列合計會漏掉 Limit Break 等來源，僅以它判斷會讓 `c6KvhDyzCNXJbRMZ` fight 7 類「角色合計 150.559m、實際敵方承傷 152.652m」資料漏網。只有已保存或新查得的 FFLogs Target Damage 可以套用前者；超過任一上限都標為 `suspected` 並隱藏，未超過則不得由此判定正常。
