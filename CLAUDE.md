...
6. `config/fight_integrity_baselines.json` 是有足夠歷史樣本的完整繁中隊伍傷害上緣；只有玩家數完整等於該場隊伍人數且未高於保守上緣時，才可離線標記為 valid。高於上緣、出現 Attack 標記或缺少完整隊伍時仍必須走敵方生命池檢核；高於上緣但生命池暫缺時，必須以 `suspected` 與 `hidden_from_public=true` 防止可疑 rDPS 回流公開排行榜。
