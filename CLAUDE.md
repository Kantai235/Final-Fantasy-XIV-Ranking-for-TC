...
7. `config/fight_integrity_known_enemy_hp.json` 與 `scripts/fight_integrity_known_capacity.py` 是比歷史預篩更優先、可獨立撤除的固定生命池下限規則。目前有極澤蓮尼亞（`92,086,232`）與幻朱雀（`127,613,543`）：完整繁中隊伍角色傷害總和超過已確認生命池的 `1.005` 倍，即標記 `suspected` 並隱藏；超過 `1.15` 倍則可標記 `excluded`。角色傷害只是敵方承傷的下限，未超過 1.005 絕不可直接寫入 `valid`。切點後缺少結果、`unverifiable` 或任何非 `valid`／`not_applicable` 狀態均採 fail-closed，不能出現在排行榜、個人成績、隊伍榜或近期動態。`backfill_fight_integrity.py --offline-only` 不建立 OAuth、不呼叫 API，只用既有快取、固定生命池和歷史基準；不能離線確認時保留 `unverifiable` 並隱藏。
