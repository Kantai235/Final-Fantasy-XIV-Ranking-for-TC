...
9. `data_integrity.calculation_version=8` 或 `9` 且狀態為 `valid`／`not_applicable`、`hidden_from_public=false` 的既有結果列入公開相容清單；但 M5S～M8S 的 v9 場次仍須逐批補上 v10 普攻分布證據。舊版 `excluded`／`suspected`／`unverifiable`、缺少結果、早於 v8 或未知版本一律 fail-closed。現行版 `integrity_measurement_failed` 屬執行期例外，後續批次必須重試；可重現的缺少生命池不應每輪重查。人工明確使用 `--force` 時可全量稽核；`--report-code` 可重複指定報告做定向複查。`.github/workflows/update_rankings.yml` 必須在推送前比對該輪起始的完整性規則檔指紋；若遠端已變更，舊 runner 必須停止而非 rebase 後回寫過期資料。
