...
5. 這是可撤除的資料品質防護。GitHub Actions 用 `FFLOGS_FIGHT_INTEGRITY_REPORT_LIMIT` 小批量逐輪補查，`FFLOGS_FIGHT_INTEGRITY_ENABLED=false` 停止新增檢核；日後 Log 工具修正後可停止 workflow 步驟與回補腳本。已標記的歷史 fight 與判定證據必須繼續保留，不得硬刪。M5S～M8S 的 v10 場次需逐批補上 v11 固定總承傷與逐目標 profile 證據；其他副本 v8～v10 已明確為 `valid`／`not_applicable` 的正常結果繼續公開。
6. `scripts/fight_integrity_cache.py` 只可保存重新套用規則所需的「彙總敵方承傷、生命池、目標數」、固定 profile 需要的「NPC GUID、逐目標承傷、最大生命值、實例數」，與不含玩家名稱的「source ID、職業、普攻命中數、純一般普攻命中數／中位數、普攻傷害／占比」最小測量資料，或可重現的無法量測原因，預設置於 `.gitignore` 排除的 `data/local-cache/fight-integrity/measurements.json`；不得保存 report 內 actor ID、完整 Target table、raw events、玩家名稱、OAuth 資訊或完整 API payload。每次成功量測或確認無法量測後須原子寫入，讓中斷不浪費已使用的 FFLogs 額度；既有 `data_integrity.metrics` 與無法量測結果必須可直接植入快取，不得為了補快取重讀 API。同一 report/fight 的來源指紋未變時，`--force` 也必須優先以快取離線重判，只有 `--refresh-cache` 或指紋變更才可再次取用 API。GitHub Actions 以 Actions cache 接續這份不進 Git 的快取。
...
9. `data_integrity.calculation_version=8`、`9` 或 `10` 且狀態為 `valid`／`not_applicable`、`hidden_from_public=false` 的既有結果列入一般公開相容清單；M5S～M8S 另由副本專用候選判斷強制將缺少 v11 `target_damage_profile` 的 v10 場次排入重驗，不影響其他副本。舊版 `excluded`／`suspected`／`unverifiable`、缺少結果、早於 v8 或未知版本一律 fail-closed。現行版 `integrity_measurement_failed` 屬執行期例外，後續批次必須重試；可重現的缺少生命池或 NPC GUID 不應每輪重查。人工明確使用 `--force` 時可全量稽核；`--report-code` 可重複指定報告做定向複查，`--encounter-key` 與 `--recorded-at-or-after` 可把人工全量稽核限縮到指定副本與含時區的 fight 紀錄時間。完整性查詢若收到 `reportData.report=null`，必須視為 report 已無法讀取並在來源節點標記 hidden；暫時性 HTTP／連線錯誤仍不得誤判為永久隱藏。`.github/workflows/update_rankings.yml` 必須在推送前比對該輪起始的完整性規則檔指紋；若遠端已變更，舊 runner 必須停止而非 rebase 後回寫過期資料。
