...
- 個人成績單載入角色後，右下角會顯示書本造型的「成就手冊」入口；手冊顯示目前適用成就、全站獲得人數與占收錄玩家比例，並標示目前角色已取得或尚未取得。零式每個量級的「首週踏破 → 次週踏破 → 踏破 → 通關」只顯示一項：玩家已取得任一階級時只顯示該成就；四項皆未取得時，依繁中服目前時間顯示仍可取得的最高階目標，截止當下仍列入。2026-08-10 的未取得玩家會看到「輕量級通關」與「次重量級【首週】踏破」；次重量級在 2026-08-11／08-18 16:00 後會依序切換為「次週踏破」／「踏破」。全站索引仍保存完整十四項成就統計；「網站作者」屬於身分標示，不納入成就總數或獲得率。
...
- 玩家比較、隊伍榜、伺服器對比、職業分析與近期動態皆由靜態資料產生；隊伍榜預設顯示「零式 M6S / 糖彩狂潮」。近期動態也提供每日 Logs 曲線、零式、極、幻、滅、絕分類占比，以及台服與國際服改版時間標註，桌面預設近 90 天、手機預設近 30 天，可切換副本、日期範圍與 Logs、通關場次等統計口徑。
