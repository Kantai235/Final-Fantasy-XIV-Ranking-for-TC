...
- 完整性 v10 會逐批補查 M5S～M8S 缺少普攻分布證據的 v9 場次；其他副本 v8／v9 已判為 `valid`／`not_applicable` 且未標記隱藏的正常戰鬥繼續公開，也不占用一般回補額度。舊版失敗結果、缺少結果、早於 v8 或未知版本仍採 fail-closed。現行版本若因執行期例外寫成 `integrity_measurement_failed`，後續批次會自動重試；可重現的缺少生命池則不會每輪重查。人工使用 `--force` 時仍可全量稽核，`--report-code` 可重複指定報告進行定向複查。workflow 推送前也會比對起始規則指紋，若執行期間規則已更新就拒絕將舊 runner 的結果回寫。
