...
- 個人成績單可查看各副本最佳紀錄、歷史紀錄、同職分位與常同場隊友；也可在查詢列右側開啟「簡表模式」，以零式、絕、極、幻、滅橫列顯示所有絕本、所有極本與目前高難副本。有效版本通關顯示跨職業最高 PR 與對應職業，只有過版紀錄則顯示灰色勾勾；「尚未收錄」不代表玩家未通關。同職分位預設顯示整數 PR 值，也可由使用者端偏好切換為「前 N%」，PR 模式會讓代表列、分位亮點與歷史列優先依 PR 值排序。
