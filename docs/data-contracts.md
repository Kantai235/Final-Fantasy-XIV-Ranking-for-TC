...
Honey B. Lovely 粉絲榜來源在 `data/fun/honey_b_fans.json`，公開檔在 `public/data/fun/honey_b_fans.json`。它是與正式排行榜分離的趣味資料，只記錄 M2S `心醉魂迷：奴役` 衍生結果與掃描快取，不參與個人成績單、全服統計或 `data/rankings/` 去重規則。
...
絕本 report 的 `checked_reports` / `processed_reports` 會記錄 `clear_rule_revision`。當通關判斷規則更新時，只要調整程式內版本，歷史補查就會把尚未套用新版本的既有絕本 report 重新選入深查；重判完成後再寫入版本，避免同一份 report 在後續輪巡中重複消耗深查預算。
