...
絕本 report 的 `checked_reports` / `processed_reports` 會記錄 `clear_rule_revision`。當通關判斷規則更新時，只要調整程式內版本與受影響副本清單，歷史補查就會把尚未套用新版本的既有受影響絕本 report 重新選入深查；目前版本只影響 UCoB。已確認沒有繁中服玩家的 report 不會因通關規則更新而重刷，因為通關判斷不會改變 `masterData` 的玩家伺服器；重判完成後再寫入版本，避免同一份 report 在後續輪巡中重複消耗深查預算。
