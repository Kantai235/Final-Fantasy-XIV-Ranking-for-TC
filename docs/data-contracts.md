...
個人成績簡表的版本選擇採「版本快照」：先用 `profile_summary_available_from` 隱藏選擇版本後才開放的副本，再以每個版本的下一個繁中服版本開放時間排除之後的 `recorded_at_iso`。因此選擇 7.0 時，極豔翼蛇鳥／極佐拉加會保留至 7.05 開放前，而 7.05 之後的同副本戰鬥不會混入。這與 `version_cutoff` 的 valid／obsolete 語意分離：前者是歷史畫面的時間範圍，後者是副本難度是否已過版。

零式在版本快照中會列出全部已開放量級；簡表預設選取 `profile_summary_savage_tier.order` 最大的一組，但可切換至較早量級。因此目前 7.05 只提供輕量級 1～4，未來次重量級開放後會同時提供輕量級與次重量級，預設顯示次重量級 1～4。某量級四層皆有該版本有效通關時，該量級按鈕會亮起彩色勾勾；量級內的第 1～4 層仍依一般簡表規則顯示職業與 PR。這只影響簡表呈現，不會刪除舊量級的個人成績或排行歷史。
