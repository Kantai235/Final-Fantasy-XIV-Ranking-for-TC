...
- 職能／職業、累積遊戲版本與歷史表格欄位排序；選定坦克或治療範圍時，可另行開啟同場同職能玩家對照。
...
坦克與治療的摘要、歷史表與報告彈窗會切換為對應支援統計。摘要列固定只顯示目前玩家；使用者開啟「同場坦克職業」或「同場治療職業」後，歷史表才會在本人紀錄下方加入唯一可辨識的另一坦／補資料列，並以原紀錄時間欄顯示「玩家名稱@伺服器」，不重複顯示報告、紀錄時間與通關時間。桌面與平板的坦補歷史紀錄在原容器寬度內維持一筆一列，以緊湊欄距與既有數值縮寫保留完整數值及原字級；手機版則維持卡片重排。Active、GCD、OH% 與減傷覆蓋等百分比會保留整數原字級，只縮小小數位，顏色則沿用完整數值原本的樣式，完整百分比不會在卡片內拆行。舊資料沒有選填欄位時顯示 `-`，前端不得自行掃描其他排行榜列補算。
...
同一零式量級先顯示互斥的五階進度，再顯示可獨立取得的炒股仔。五階依「首週踏破 → 次週踏破 → 首月踏破 → 當版本踏破 → 通關」只發放命中的最高階：

| 量級 | 首週 | 次週 | 首月 | 當版本 | 通關 |
| --- | --- | --- | --- | --- | --- |
| 輕量級 | M4S 於 2026-03-17 16:00 前完成 | M4S 於 2026-03-24 16:00 前完成 | M4S 於 2026-04-07 16:00 前完成 | M4S 於 2026-06-23 13:00 前完成 | M1S～M4S 皆有有效版本公開成績 |
| 次重量級 | M8S 於 2026-08-11 16:00 前完成 | M8S 於 2026-08-18 16:00 前完成 | M8S 於 2026-09-01 16:00 前完成 | 現行依序攻略限制下，M8S 有有效公開通關 | M5S～M8S 皆有有效版本公開成績，且未命中更高階 |

門檻當下列入。實際完成時間以戰鬥紀錄時間加通關時間計算；缺少可解析時間時不猜測限時階級，但四層完整仍可取得通關。次重量級日後解除依序攻略限制時，只能以明確公告切點更新固定規則，不可由副本掃描日期自動推導。

當版本成就的名稱為「輕量級【當版本】踏破」與「次重量級【當版本】踏破」，說明皆為「沒關係，次週首月當版本通關也很厲害了」，其中「次週」與「首月」各自以 X 樣式劃掉。成就卡片與手冊共用 `AchievementDescription.vue` 呈現工具層的說明片段，靜態索引仍保存純文字；名稱與說明使用程式內的固定目錄，避免舊資料快照覆蓋最新文案。成就 ID、判定門檻與獲得人數的計算方式不變。

每個量級另有獨立的炒股仔成就：四層各自至少有一筆非過版、`performance.qualified=true` 且本站四捨五入顯示 PR 95 以上的紀錄，各層可以使用不同職業。輕量級使用固定有效版本成績；次重量級會隨現行分位重建。

成就手冊分為絕本、零式、其他三頁，完整目錄有 18 列。進度計算把每個量級的五階視為一項，兩個炒股仔各自獨立，因此總進度共有 10 項。全站獲得人數與百分比由 `scripts/build_user_data.mjs` 以完整角色公開成績建置到 `users/index.json`；分母固定為同一索引的 `total_users`，前端不得掃描所有玩家檔重算。
