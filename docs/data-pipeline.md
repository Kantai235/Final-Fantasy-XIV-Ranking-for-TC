...
`scripts/fetch_honey_b_fans.py` 與正式排行榜分離，固定使用 `savage_m2s` 的 zone / encounter / difficulty 設定掃描 Honey B. Lovely 粉絲紀錄。它只保存通關與 wipe 場次中進入 `心醉魂迷：奴役` 的衍生資料、已檢查戰鬥狀態與 report 快取，來源檔是 `data/fun/honey_b_fans.json`，公開檔是 `public/data/fun/honey_b_fans.json`。
