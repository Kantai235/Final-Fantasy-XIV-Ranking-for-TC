...
function validateUserProfileBadges() {
  ...
  for (const 名稱 of ["輕量級通關", "次重量級踏破", 六絕踏破稱號, "多職玩家", "三色豆", "高活躍", "社群核心", "近期活躍"]) {
  ...
  const 預期前四枚徽章 = ["網站作者", 六絕踏破稱號, "次重量級踏破", "輕量級通關"];
  assert(
  ...
  assert(
    ...
    "6/23 13:00 後只有 M4S 紀錄時，不可跳過四層條件取得輕量級踏破。",
  );
  驗證量級互斥徽章(
    "輕量級",
    建立量級成績(
  ...
  assert(
    JSON.stringify(六絕含過版紀錄.slice(0, 3).map((徽章) => 徽章.名稱))
      === JSON.stringify([六絕踏破稱號, "次重量級踏破", "輕量級通關"]),
