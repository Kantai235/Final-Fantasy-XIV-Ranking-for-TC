...
function validateRankingDefaults() {
  assert(預設副本鍵值 === "savage_m8s", "排行榜目前必須預設顯示零式 M8S／呼嘯之劍。");
  assert(預設隊伍榜副本鍵值 === "savage_m8s", "隊伍榜目前必須預設顯示零式 M8S／呼嘯之劍。");
}
...
function validateUserProfileBadges() {
  ...
  for (const 名稱 of ["輕量級通關", "次重量級【當版本】踏破", 六絕踏破稱號, "多職玩家", "三色豆", "高活躍", "社群核心", "近期活躍"]) {
  ...
  const 預期前四枚徽章 = ["網站作者", 六絕踏破稱號, "次重量級【當版本】踏破", "輕量級通關"];
  assert(
  ...
  assert(
    JSON.stringify(含炒股仔的量級徽章.map((項目) => 項目.名稱))
      === JSON.stringify(["次重量級【當版本】踏破", "次重量級的炒股仔", "輕量級通關", "輕量級的炒股仔"]),
  ...
  驗證量級互斥徽章(
    "輕量級",
    建立量級成績(["savage_m4s"], "2026-03-17T08:00:00.000Z"),
    "輕量級【首週】踏破",
  );
  驗證量級互斥徽章(
    "輕量級",
    建立量級成績(["savage_m4s"], "2026-03-17T08:00:00.001Z"),
    "輕量級【次週】踏破",
  );
  驗證量級互斥徽章(
    "次重量級",
    建立量級成績(["savage_m8s"], "2026-08-11T08:00:00.000Z"),
    "次重量級【首週】踏破",
  );
  驗證量級互斥徽章(
    "次重量級",
  ...
  // 列入。晚 1ms 後不能再取得踏破，只有 M1S～M4S 四層皆有效時才改發通關。
  驗證量級互斥徽章(
    "輕量級",
    建立量級成績(["savage_m4s"], "2026-06-23T05:00:00.000Z"),
    "輕量級【當版本】踏破",
  );
  const 解除限制後只有M4S = 建立零式量級踏破徽章(
    建立量級成績(["savage_m4s"], "2026-06-23T05:00:00.001Z"),
  );
  assert(
    !解除限制後只有M4S.some((徽章) => 徽章.名稱.startsWith("輕量級")),
    "6/23 13:00 後只有 M4S 紀錄時，不可跳過四層條件取得輕量級【當版本】踏破。",
  );
  驗證量級互斥徽章(
    "輕量級",
    建立量級成績(
      輕量級副本鍵值,
      "2026-06-23T05:00:00.001Z",
      "2026-06-23T05:00:00.001Z",
    ),
    "輕量級通關",
  );
  ...
  驗證量級互斥徽章(
    ...
    [{
      ...
      job: "Paladin",
      recorded_at_iso: null,
      clear_time_ms: null,
    }],
    "次重量級【當版本】踏破",
  );

  const 缺少通關時間的末層 = 建立零式量級踏破徽章([{
    encounter_key: "savage_m4s",
    job: "Paladin",
    recorded_at_iso: "2026-03-17T07:50:00.000Z",
    clear_time_ms: null,
    clear_time_seconds: null,
  }]);
  assert(
    !缺少通關時間的末層.some((徽章) => 徽章.名稱.startsWith("輕量級")),
    "M4S 缺少可解析的通關時間時，不可把 null 當成 0 毫秒而猜測限時成就。",
  );
  ...
  assert(
    ...
    "沒有 M8S 有效版本公開成績時，不可發放次重量級【當版本】踏破徽章。",
  );
  ...
  assert(
    JSON.stringify(六絕含過版紀錄.slice(0, 3).map((徽章) => 徽章.名稱))
      === JSON.stringify([六絕踏破稱號, "次重量級【當版本】踏破", "輕量級通關"]),
