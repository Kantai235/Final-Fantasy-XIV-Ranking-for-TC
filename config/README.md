...
- `max_runtime_seconds` 與 `runtime_grace_seconds` 控制 `fetch_fflogs.py` 的執行時間預算。專案預設 `max_runtime_seconds=0` 代表本機不限制；GitHub Actions 預設 `FFLOGS_MAX_RUNTIME_SECONDS=6000`、`FFLOGS_RUNTIME_GRACE_SECONDS=900`，讓抓取步驟約 85 分鐘內主動收尾，保留時間給資料建置、commit 與部署。時間不足時腳本會保留 `active_scan` 與已落地批次，不會推進未完成副本掃描點。
