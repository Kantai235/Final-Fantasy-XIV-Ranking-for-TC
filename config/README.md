...
- `max_runtime_seconds` 與 `runtime_grace_seconds` 控制 `fetch_fflogs.py` 的可選執行時間預算。專案預設 `max_runtime_seconds=0` 代表不限制；正式 GitHub Actions 也不設定這組環境變數，確保所有副本、Honey B. Lovely 粉絲榜抓取與既有 report GCD 回補都能依序嘗試執行。若人工短時維護需要限制抓取時間，可臨時設定 `FFLOGS_MAX_RUNTIME_SECONDS` 與 `FFLOGS_RUNTIME_GRACE_SECONDS`；時間不足時腳本會保留 `active_scan` 與已落地批次，不會推進未完成副本掃描點。
