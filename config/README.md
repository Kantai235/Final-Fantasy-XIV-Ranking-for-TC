...
- v9 專門重判 v8 失敗案例：`data_integrity.calculation_version=8` 且為 `valid`／`not_applicable`、未標記隱藏的 fight 繼續公開，也不進入一般回補候選；v8 的 `excluded`／`suspected`／`unverifiable` 才逐批升級至 v9。缺少結果、早於 v8 或其他未知舊版仍採 fail-closed。回補時會先從失敗結果的最小量測欄位植入本機快取，再以現行規則離線重判；人工 `--force` 可明確要求全量稽核。workflow 在推送前會比對本輪起始的完整性規則檔指紋，防止長時間舊 runner 回寫過期結論。
