...
- `fight_integrity_check` 是可撤除的暫時性資料品質防護，專門檢查台灣時間 2026-07-28 18:00 後受普攻解析問題影響的 fight。`cutoff_iso` 是時區明確的啟用切點，`hp_ratio_threshold=1.15` 代表全隊對敵方目標造成的傷害超過其最大生命池總和 15% 時，以 `data_integrity.status="excluded"` 隱藏；`damage_done_summary.exploitDetails` 出現 `guid=7`／`Attack` 時，即使未達門檻也會以 `status="suspected"` 隱藏。泛用 `exploit:6` 不參與判定。`excluded_encounter_keys` 用於生命池語意不適用的多階段副本（目前為 `ultimate_bahamut`），避免誤判。停用 `enabled` 或 workflow 的 `FFLOGS_FIGHT_INTEGRITY_ENABLED=false` 只停止新增檢核，既有標記仍會持續從公開產物隱藏。
