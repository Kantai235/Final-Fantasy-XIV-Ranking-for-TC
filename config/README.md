...
- 一般個人成績單的量級踏破另有固定限時成就，規則集中於 `src/utils/userProfileBadges.js`，不從 `scan_start_date` 或簡表量級欄位動態推算。輕量級以繁中服時間 2026-03-17／03-24 16:00 作為首週／次週截止，次重量級以 2026-08-11／08-18 16:00 作為首週／次週截止；截止當下仍列入。四層都必須各有一筆有效版本公開成績的「`recorded_at_iso` 戰鬥開始時間 + `clear_time_ms` 通關時間」落在期限內，同量級只顯示首週、次週、一般三者中最高階的一枚；時間缺失或無法解析時只能取得一般踏破徽章。
...
- v8～v10 的 `valid`／`not_applicable`、未標記隱藏 fight 保留一般公開相容性；M5S～M8S 由副本專用候選判斷額外挑出缺少 v11 `target_damage_profile` 的場次，不會讓其他副本因全域版號升級整批下架。舊版 `excluded`／`suspected`／`unverifiable`、缺少結果、早於 v8 或其他未知版本仍採 fail-closed。回補時會先從既有結果的最小量測欄位植入本機快取，再以現行規則離線重判；人工 `--force` 可明確要求全量稽核。workflow 在推送前會比對本輪起始的完整性規則檔指紋，防止長時間舊 runner 回寫過期結論。
