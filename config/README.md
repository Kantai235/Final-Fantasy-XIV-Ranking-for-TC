...
- 極澤蓮尼亞的 `required_enemy_damage_min`／`required_enemy_damage_max` 與角色全隊傷害固定範圍相同，但只在 `players[]` 未完整涵蓋隊伍時，改用既有量測快取或 FFLogs Target Damage 的敵方承傷值比對。此例外不能套用至幻朱雀等多目標副本，避免將敵方承傷錯當作角色總傷害。
