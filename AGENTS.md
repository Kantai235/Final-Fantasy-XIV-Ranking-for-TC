...
25. GitHub Actions 會在同步 users 專用 repo、執行 Vite/postbuild 後，先跑 `npm run prune:pages-user-data` 保留 `dist/data/users/index.json`，並移除 `dist/data/users` 內的個別玩家檔、`dist/data/user-entry-details`、hidden 使用者差量 JSON、逐玩家靜態分享頁與 `dist/og/users`，再於上傳 Pages artifact 前執行 `npm run audit:pages-payload:strict -- --write-history data/pages_payload_history.jsonl`。正式 workflow 以 `FFXIV_TC_BUILD_USER_SHARE_PAGES=false` 關閉逐玩家靜態分享頁與玩家 OG 圖，只保留 `/user` route 層級入口與共用搜尋索引；`dist/data/users/` 的 target 仍保留給本機完整 build、緊急排查或流程異動時監控。排行榜資料會先以滾動快照推送，讓 payload 超標時只停止部署而不遺失 FFLogs 成果；稽核通過且 `data/pages_payload_history.jsonl` 有變更時，workflow 會 amend 同一筆資料快照，不另外追加 payload commit。`npm run audit:pages-payload` 只保留作為本機 baseline 觀察用途。
...
28. `.github/workflows/update_rankings.yml` 與 `.github/workflows/emergency_deploy.yml` checkout 時只抓目前分支的淺層 partial clone，後續同步也只 fetch 有限深度。正式排行榜 workflow 為安全 amend 資料快照必須至少取得 HEAD 與 parent 兩層，不得降為 `fetch-depth: 1`；資料 repo 的完整歷史 pack 已累積到數十 GiB，也不得改為 `fetch-depth: 0`，避免 GitHub-hosted runner 在 checkout 階段耗盡磁碟。
...
31. 主 repo 的自動化資料 commit 使用 `scripts/commit_workflow_data_snapshot.mjs` 統一為 `chore(data): 更新自動化資料快照`，並在 commit body 保存 `Workflow-Data-Snapshot: v1` 標記。排行榜資料會先推送，payload 稽核後再 amend 同一快照；後續排程只能改寫 HEAD 上帶標記的快照，並以明確舊 SHA 的 `force-with-lease` 更新。遠端有新程式碼 commit 時必須停止，不得 rebase 後強制覆寫。人工程式碼 commit 是新的歷史分界；已成為後續程式碼 parent 的舊資料 commit 不得由日常 workflow 改寫，既有容量只能另行一次性歷史清理。
