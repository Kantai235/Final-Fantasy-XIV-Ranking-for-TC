...
8. 量級首週、次週與踏破徽章的視覺由 `量級踏破徽章`、量級與階級三組語意類別組合：輕量級使用傾斜的「輕」背景字、次重量級使用「次」；三階分別採金、銀、銅色，掃光與光暈強度依序降低。「通關」使用預設徽章外觀，不套用量級背景字、掃光或光暈。動畫必須尊重 `prefers-reduced-motion`，亮色主題也需維持文字對比；不得在 Vue 元件內依成就名稱重做樣式判斷。
...
10. 個人成績單的「成就手冊」與既有徽章共用 `src/utils/userProfileBadges.js` 的固定成就 ID 與判定規則；網站作者是身分標示，不納入成就目錄。`scripts/build_user_data.mjs` 必須以完整角色公開成績、完整同場玩家數與最新公開紀錄時間聚合全站獲得人數，將四階量級成就與其他一般成就的完整 `achievements[].holder_count`／`holder_percentage` 寫入 `users/index.json`；占比分母固定使用同一索引的 `total_users`，Vue 只能讀取該靜態統計並標示目前角色是否取得，不得掃描所有玩家檔案重算。手冊固定分為「絕本」、「零式」、「其他」三個分頁；零式分頁依量級完整顯示「首週踏破 → 次週踏破 → 踏破 → 通關」四階的全站人數與百分比，但四階仍為互斥成就，角色只會取得最高階一枚。四階皆未取得時，仍依目前繁中服時間標記尚可取得的最高階為「目前目標」，且截止當下仍列入；其他三階只能標為「其他階段」，不可誤示為可重複取得。總進度與分頁進度必須使用工具層的進度群組計算，每個零式量級只占一項，因此完整顯示十四列後總數仍為八項。分類、量級群組與進度群組必須集中在工具層，不得在 Vue 元件內依名稱判斷；彈窗外框高度固定，僅內容區捲動，避免三個分頁因列數不同而跳動。
