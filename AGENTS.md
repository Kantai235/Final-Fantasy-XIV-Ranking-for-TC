...
7. `config/fight_integrity_known_enemy_hp.json` 與 `scripts/fight_integrity_known_capacity.py` 是比歷史預篩更優先、可獨立撤除的固定規則。極澤蓮尼亞確認完整繁中隊伍的 `players[].total_damage` 正常總和必須落在 `92,086,132–92,086,332`；完整隊伍低於或高於這個範圍一律標記異常並隱藏，範圍內且沒有 `Attack` 標記可直接寫入 `valid`，無須讀取 FFLogs API。幻朱雀的四目標承傷沒有相同群組，仍只使用固定生命池 `127,613,543` 的下限：完整隊伍角色傷害總和超過其 `1.005` 倍即標記 `suspected` 並隱藏，超過 `1.15` 倍則可標記 `excluded`；未超過只表示未命中，不可直接寫入 `valid`。絕伊甸、絕巴哈姆特、M1、M3、M4 的完整隊伍總傷害高於 `151,500,000`、`13,230,230`、`75,870,000`、`96,523,000`、`114,526,000` 時，一律標示為 `suspected` 並隱藏。這些是只有超標才成立的獨立硬上限，不能將未超標資料寫成 `valid`；它們在多階段副本的生命池 `not_applicable` 之前執行，避免 243m 類異常藉此例外重新出現在公開資料。切點後缺少結果、`unverifiable` 或任何非 `valid`／`not_applicable` 狀態均採 fail-closed，不能出現在排行榜、個人成績、隊伍榜或近期動態。`backfill_fight_integrity.py --offline-only` 不建立 OAuth、不呼叫 API，只用既有快取、固定規則和歷史基準；不能離線確認時保留 `unverifiable` 並隱藏。
