...
* **禁止覆寫或硬刪除**：嚴禁寫出會隨意覆蓋或刪除歷史紀錄的程式碼。同步 Data repo 前必須先執行 `npm run sync:data -- --dry-run`；只要本機受管理資料與遠端 snapshot 不同，hydrate 就必須停止，不能用 `--force` 掩蓋未知差異。發布時也必須通過既有 report、fight、player 與 checkpoint 的 append-only 守恆檢查。
...
7. `Final-Fantasy-XIV-Ranking-for-TC-Data` 是 `data/` 權威來源與主站共用 `public/data/` 快照 repo；主 repo 只追蹤程式碼與設定。任何本機抓取、資料建置或 workflow 都必須先以 `scripts/data_repository.mjs hydrate` 驗證 manifest 並還原最新快照。Data repo 只保存 `public/data/` 根層 JSON 與 `public/data/fun/`，可重建的排行榜薄索引、hidden delta、個別玩家與報告明細不得重複保存。
...
6. 7.2 確定於繁中服 2026-07-28 13:00 開放；`extreme_zelenia` 使用 Trials II (Extreme) `zone_id=67`、Zelenia `encounter_id=1080`、`difficulty=100`，並以 `scan_start_date="2026-07-28T13:00:00+08:00"` 排程。`savage_m5s` 至 `savage_m8s` 使用 AAC Cruiserweight `zone_id=68`、`encounter_id=97` 至 `100`、`difficulty=101`；次重量級因資料收錄排程調整，四層的 `scan_start_date` 統一延至 `2026-08-04T13:00:00+08:00`。此延後只控制 FFLogs 掃描，不影響 7.2 遊戲版本切點或個人成績簡表量級。未到各自掃描開放時間的啟用副本不得查詢 FFLogs，也不得在公開清單出現；公開清單只可列入已有排行榜檔案的副本，以避免前端讀取空路徑。同一時間 `savage_m1s` 至 `savage_m4s`、`extreme_queen_eternal` 與 `chaotic_cloud_of_darkness` 以 `version_cutoff` 標記過版；`unreal_byakko` 用 `scan_end_date` 停止新增掃描，但用 `profile_summary_available_until="7.2"` 保留至 7.2 簡表，`unreal_suzaku` 則使用 Trials (Unreal) `zone_id=64`、`encounter_id=3010`、`difficulty=100` 自 7.2 起掃。7.2 個人成績簡表的幻本必須固定同時顯示白虎與朱雀；即使玩家沒有白虎公開成績，白虎仍須顯示為「尚未收錄」。
7. 個人成績一般模式的零式量級成就採「首週踏破 → 次週踏破 → 踏破 → 通關」四階互斥，只顯示同量級最高階的一枚，並使用未套用頁面篩選的完整公開成績與非 `is_obsolete_record` 紀錄。輕量級首週／次週以 M4S 在繁中服時間 2026-03-17／03-24 16:00 前完成為準；「輕量級踏破」要求在 2026-06-23 13:00 前完成 M4S，「輕量級通關」則要求 M1S～M4S 四層各有有效版本公開成績。次重量級首週／次週以 M8S 在 2026-08-11／08-18 16:00 前完成為準；目前尚未解除依序攻略樓層的限制，因此「次重量級踏破」只要求 M8S 有效公開通關，「次重量級通關」要求 M5S～M8S 四層各有有效版本公開成績，但只有在較高階踏破未命中時才會發放。上述截止當下均列入，時間判定以 `recorded_at_iso`（舊資料可退回 `recorded_at`）加上 `clear_time_ms`（可退回 `clear_time_seconds`）計算實際完成時間；缺少可解析時間時不得猜測首週、次週或輕量級踏破，但四層完整仍可取得通關。固定規則集中在 `src/utils/userProfileBadges.js`；每個量級須明確保存末層鍵值與可為 `null` 的「末層可代表全層截止時間」，次重量級未來解除限制時只需填入公告切點，切點後的 M8S 不再取得踏破，只有 M5S～M8S 四層完整者取得通關，不可由 `profile_summary_savage_tier` 或 `scan_start_date` 自動衍生。
8. 量級成就以 `量級成就徽章` 搭配輕／次量級語意類別共用「輕」與「次」背景字。首週、次週與踏破另組合 `量級踏破徽章` 及金、銀、銅階級類別，掃光與光暈強度依序降低；「通關」使用預設徽章外觀。炒股仔另組合 `炒股仔徽章`，依台灣股市紅漲慣例使用橫跨可用版面的紅色粗上升折線與大型實心箭頭，箭頭必須停在右側「輕／次」量級字左側，不套用踏破掃光；走勢依折線轉折點由左往右逐段揭露，完整停留後淡出並循環。動畫必須尊重 `prefers-reduced-motion`，亮色主題也需維持文字對比；不得在 Vue 元件內依成就名稱重做樣式判斷。
9. 個人成績徽章的顯示順序固定為：唯一的網站作者識別置頂；六絕全通是一般成就中的最高優先；次重量級第二；輕量級第三；其餘一般成就接續顯示。同一量級先顯示命中的四階進度，再顯示可獨立取得的炒股仔。非作者角色從六絕全通開始。量級排序必須使用 `src/utils/userProfileBadges.js` 內的明確優先值，不可依名稱、開放日期或 Vue 元件中的條件判斷重排。
10. 個人成績單的「成就手冊」與既有徽章共用 `src/utils/userProfileBadges.js` 的固定成就 ID 與判定規則；網站作者是身分標示，不納入成就目錄。`scripts/build_user_data.mjs` 必須以完整角色公開成績、完整同場玩家數與最新公開紀錄時間聚合全站獲得人數，將完整十六列成就的 `achievements[].holder_count`／`holder_percentage` 寫入 `users/index.json`；占比分母固定使用同一索引的 `total_users`，Vue 只能讀取該靜態統計並標示目前角色是否取得，不得掃描所有玩家檔案重算。手冊固定分為「絕本」、「零式」、「其他」三個分頁；零式分頁每個量級分成「四階進度」與「額外成就」。四階完整顯示「首週踏破 → 次週踏破 → 踏破 → 通關」的全站人數與百分比，但仍為互斥成就，角色只會取得最高階一枚；完全未取得時依目前繁中服時間標記尚可取得的最高階為「目前目標」，其他三階標為「其他階段」。`savage-light-heavyweight-stock-trader` 與 `savage-cruiserweight-stock-trader` 為可獨立取得的炒股仔成就：各量級四層都必須有非 `is_obsolete_record`、`performance.qualified=true` 且本站四捨五入顯示 PR 95 以上的紀錄，各層可以使用不同職業。輕量級有效版本成績固定；次重量級會隨現行有效版本分位重建而動態變化。總進度與分頁進度使用工具層的進度群組計算，每個零式四階量級只占一項，兩項炒股仔各自獨立，因此完整目錄十六列、總數十項。分類、量級群組、項目類型與進度群組必須集中在工具層，不得在 Vue 元件內依名稱判斷；彈窗外框高度固定，僅內容區捲動，避免三個分頁因列數不同而跳動。
...
16. 新收錄 fight 的玩家批次查詢必須同時取得 Healing table，並以 `scripts/fflogs_pipeline/support_metrics.py` 將補師整理為 `healing_stats`、坦克整理為 `tank_stats`；坦克存在時另完整分頁取得 DamageTaken、友方 Buffs 與敵方 Debuffs events。只保存版本化摘要與 fight 層 `support_metrics_summary`，不得落地 raw events。OH% 固定以純治療加過量治療為分母；個人／團隊防護是實際消耗護盾量。減傷覆蓋只使用全副本共用的玩家 Status ID 規則，activation 有實際傷害落在狀態時窗內才算有效，重疊時取傷害事件聯集，不能宣稱為單招實際減免量。FFLogs namespaced Status ID 必須先還原，團隊 Buff 的多 target 封包必須收斂為一次 activation，但單體充能技能不得因時窗重疊而合併。同場另一坦／補必須留待 Node.js 建置層依 fight 關聯，Python 不得寫入 UI 專用搭檔欄位。`scripts/build_ranking_table_data.mjs` 只在同一 fight 恰有兩名同職能玩家時輸出雙向 `co_tank`／`co_healer` 摘要；同場超過兩名同職能玩家且沒有小隊分組資訊時必須輸出 `null`，不得猜測配對。`scripts/build_user_data.mjs` 必須把坦補支援統計收斂為個人成績與報告變體的選填欄位；個人成績只需要保存 `co_healer`，而且同樣僅能在同場恰有兩名補師時建立，避免聯盟副本誤配。
17. 坦補支援統計歷史資料採雙軌回補：本機 `npm run backfill:support` 固定重抓繁中服 7.2 開放時間 `2026-07-28T05:00:00Z` 至執行開始時間的缺漏 report，依最舊優先並每 25 份寫入檢查點；GitHub Actions 則以同一時間為 `support_metrics_report_backfill.cutoff_sort_time`，每輪從 cursor 由新往舊處理切點以前的 report。游標、失敗重試清單、計算版本與減傷規則版本保存在 `data/state.json`；版本變更時保留固定切點但從切點重新回補。這套回補不得推進一般 report 掃描點，也不得保存 raw Healing table 或 events。
...
3. 本機開始產生資料前必須先執行 `npm run sync:data -- --dry-run`；本機已有不同資料時不得自動覆寫。要開始新一輪時先暫停 workflow，確認並保存本機成果，再從最新 Data repo snapshot 還原。發布工具必須以 manifest、append-only 守恆檢查與 `force-with-lease` 阻止遺失及競爭覆寫。
...
27. GitHub Actions 會在 `fetch_fflogs.py` 後執行 `npm run audit:mixed-report-dispatch`，把 mixed report 分派版本覆蓋率、待重查副本-report 組合、ranking-only 待補項目、歷史補查游標與 deferred 數量輸出到 Step Summary。此報表只作觀測用途，pending 大於 0 不應阻擋 Data snapshot 發布或部署；若要判斷歷史混合上傳重掃是否追平，應看「待重查」是否歸零，並搭配各副本歷史游標是否完成全區間輪巡。
28. `.github/workflows/update_rankings.yml` 與 `.github/workflows/emergency_deploy.yml` checkout 主 repo 時只抓目前程式碼 commit 的淺層 partial clone，再由 Data repo 的單一 root snapshot 還原權威資料；不得改為 `fetch-depth: 0`，避免重新下載遷移前累積到數十 GiB 的資料歷史。正式更新與緊急部署都必須在任何資料建置前完成 hydrate 與 manifest 驗證。
...
31. Data repo 的自動化資料發布統一由 `scripts/data_repository.mjs` 負責。每次 snapshot 必須是沒有 parent 的 root commit，manifest 必須記錄每個受管理檔案的大小與 SHA-256，並以明確舊 SHA 的 `force-with-lease` 更新 `main`。發布前必須確認舊快照的 report、`fight_id`、玩家身分與 checked-report checkpoint 都仍存在；遠端 Data snapshot 或主 repo 程式碼若在本輪更新，舊 runner 必須停止。workflow 先在 Pages 建置前發布 FFLogs 成果，payload 稽核寫入趨勢後再發布一次，不得把資料 commit 寫回主 repo。
...
6. `npm run build:honey-fans` 只由既有來源檔重建公開 JSON，不呼叫 FFLogs API；正式 workflow 會在資料建置階段執行它，並把 `public/data/fun/*.json` 納入 Data repo snapshot。`npm run validate:data` 與 `npm run test:frontend-data` 會檢查公開粉絲榜資料契約。
...
4. `ultimate_bahamut` 的多階段敵方生命池語意不能套用此倍率檢核，必須寫入 `not_applicable`，避免把正常歷史戰鬥誤判。其他無法取得最大 HP 的 fight 只能寫 `unverifiable`，除非同時有 `Attack` 標記，或已超過歷史傷害預篩上緣，才可隱藏為 `suspected`。
5. 這是可撤除的資料品質防護。GitHub Actions 用 `FFLOGS_FIGHT_INTEGRITY_REPORT_LIMIT` 小批量逐輪補查，`FFLOGS_FIGHT_INTEGRITY_ENABLED=false` 停止新增檢核；日後 Log 工具修正後可停止 workflow 步驟與回補腳本。已標記的歷史 fight 與判定證據必須繼續保留，不得硬刪。M5S～M8S 需逐批補上 v11 固定總承傷與逐目標 profile 證據；幻朱雀需補上 v13 全職業 ability 7／8 普攻基準證據。其他副本 v8～v12 已明確為 `valid`／`not_applicable` 的正常結果繼續公開。
6. `scripts/fight_integrity_cache.py` 只可保存重新套用規則所需的「彙總敵方承傷、生命池、目標數」、固定 profile 需要的「NPC GUID、逐目標承傷、最大生命值、實例數」，與不含玩家名稱的「ability ID、source ID、職業、普攻命中數、純一般普攻命中數／中位數、普攻傷害／占比／每秒傷害」最小測量資料，或可重現的無法量測原因，預設置於 `.gitignore` 排除的 `data/local-cache/fight-integrity/measurements.json`；不得保存 report 內 actor ID、完整 Target table、raw events、玩家名稱、OAuth 資訊或完整 API payload。每次成功量測或確認無法量測後須原子寫入，讓中斷不浪費已使用的 FFLogs 額度；既有 `data_integrity.metrics` 與無法量測結果必須可直接植入快取，不得為了補快取重讀 API。同一 report/fight 的來源指紋未變時，`--force` 也必須優先以快取離線重判，只有 `--refresh-cache`、缺少規則指定的 ability ID 或指紋變更才可再次取用 API。GitHub Actions 以 Actions cache 接續這份不進 Git 的快取。
7. `config/fight_integrity_baselines.json` 與 `scripts/fight_integrity_baselines.py` 是可獨立移除的歷史預篩層。它只使用 2026-07-28 18:00 前、`fight_hash` 去重後、完整繁中隊伍的 `players[].total_damage` P99；此欄位會受 Limit Break、Pet 與多目標傷害影響，絕不是 Boss HP。舊副本完整隊伍低於 P99×1.05 時可直接寫入本地 `valid`，節省生命池 API；高於上緣時必須繼續查快取或 FFLogs 生命池，基準本身不得寫入 `excluded`。極澤蓮尼亞、幻朱雀與樣本不足的副本不放入基準設定，仍完全走生命池／Attack 路徑。
8. `config/fight_integrity_known_enemy_hp.json` 與 `scripts/fight_integrity_known_capacity.py` 是比歷史預篩更優先、可獨立撤除的固定規則。極澤蓮尼亞要求完整繁中隊伍的 `players[].total_damage` 總和落在 `92,086,132–92,086,332`；幻朱雀依站務暫時性收錄決定，要求落在 `71,280,000–72,720,000`（72m ±1%）。兩者在玩家合計落於範圍內且沒有 `Attack` 標記時可直接寫入 `valid`，高於上限時可直接標記異常；低於下限只代表玩家傷害下限不足，因為 Limit Break 不會歸屬 `players[]`，必須改以既有快取或 FFLogs Target Damage 比較相同範圍。當 `players[]` 未完整涵蓋隊伍時也使用同一敵方承傷路徑，不可因玩家列缺人而跳過固定總傷害規則。幻朱雀的固定生命池 `127,613,543` 仍保留作為可追溯量測資訊，但固定範圍優先於下限倍率。絕伊甸、絕巴哈姆特、M1、M3、M4 的完整隊伍總傷害高於 `151,500,000`、`13,230,230`、`75,870,000`、`96,523,000`、`114,526,000` 時，一律標示為 `suspected` 並隱藏。這些是只有超標才成立的獨立硬上限，不能將未超標資料寫成 `valid`；它們在多階段副本的生命池 `not_applicable` 之前執行，避免 243m 類異常藉此例外重新出現在公開資料。切點後缺少結果、`unverifiable` 或任何非 `valid`／`not_applicable` 狀態均採 fail-closed，不能出現在排行榜、個人成績、隊伍榜或近期動態。`backfill_fight_integrity.py --offline-only` 不建立 OAuth、不呼叫 API，只用既有快取、固定規則和歷史基準；不能離線確認時保留 `unverifiable` 並隱藏。
9. `data_integrity.calculation_version=8` 至 `12` 且狀態為 `valid`／`not_applicable`、`hidden_from_public=false` 的既有結果列入一般公開相容清單；M5S～M8S 另依 profile version 將缺少 v11 `target_damage_profile` 的場次排入重驗，幻朱雀則依普攻 reference version 與 ability ID 清單將缺少 v13 跨職業基準的場次排入重驗，不影響其他副本。舊版 `excluded`／`suspected`／`unverifiable`、缺少結果、早於 v8 或未知版本一律 fail-closed。現行版 `integrity_measurement_failed` 屬執行期例外，後續批次必須重試；可重現的缺少生命池或 NPC GUID 不應每輪重查。人工明確使用 `--force` 時可全量稽核；`--report-code` 可重複指定報告做定向複查，`--encounter-key` 與 `--recorded-at-or-after` 可把人工全量稽核限縮到指定副本與含時區的 fight 紀錄時間。完整性查詢若收到 `reportData.report=null`，必須視為 report 已無法讀取並在來源節點標記 hidden；暫時性 HTTP／連線錯誤仍不得誤判為永久隱藏。`.github/workflows/update_rankings.yml` 必須在推送前比對該輪起始的完整性規則檔指紋；若遠端已變更，舊 runner 必須停止而非 rebase 後回寫過期結論。
10. 極澤蓮尼亞與幻朱雀的 `required_enemy_damage_min`／`required_enemy_damage_max` 是完整隊伍角色列不完整，或完整玩家合計低於固定下限時的總傷害替代證據：只能使用既有快取或 FFLogs Target Damage 的敵方承傷，分別比對 `92,086,132–92,086,332` 與 `71,280,000–72,720,000` 範圍。這是副本個別明示的例外，不得推廣到其他多目標副本；未設定此範圍者仍只能使用角色全隊傷害規則或生命池量測。
11. 絕伊甸的暫時性 `maximum_enemy_damage=152,000,000` 與仍為 `151,500,000` 的 `maximum_full_party_damage` 是互補且分開校準的單向上限：玩家列合計會漏掉 Limit Break 等來源，僅以它判斷會讓 `c6KvhDyzCNXJbRMZ` fight 7 類「角色合計 150.559m、實際敵方承傷 152.652m」資料漏網。只有已保存或新查得的 FFLogs Target Damage 可以套用前者；超過任一上限都標為 `suspected` 並隱藏，未超過則不得由此判定正常。敵方承傷上限變更時，回補器只重判既有量測高於新舊門檻中較低者的戰鬥，避免為門檻校準重新查詢全部絕伊甸 report。
12. v11 對 M5S～M8S 使用 `config/fight_integrity_known_enemy_hp.json` 的固定總承傷範圍與 `target_damage_profile`。逐目標證據必須以 NPC GUID 而非 report 內 actor ID 對應，並核對最大生命值、等效承傷實例數與轉場比例；M8S 的兩隻狼各只計 40% 最大生命值。總量與逐目標任一不符都會標為 `suspected` 並隱藏，避免不同目標間的錯誤傷害剛好互相抵銷後通過總量檢核。
13. v13 對幻朱雀全部 21 個戰鬥職業使用 `config/fflogs.json` 的副本／職業專屬普攻分布。ability 7 涵蓋多數職業的 Attack，ability 8 補齊吟遊詩人／機工士的 Shot；每個職業都保存人工抽樣的純一般普攻每擊中位數、普攻占比與普攻每秒傷害。單一玩家必須同時跨過三項門檻才標為 `suspected`：物理職業預設為參考值的 `2×／1.5×／1.5×`，法系與治療職另有每擊 `1,000`、占比 `2%`、每秒傷害 `100` 的絕對下限，避免正常 1 點 packet、單一尖峰或職業本身的高普攻傷害造成誤判。這條明示單人規則不得套用到其他副本，也不得依玩家名稱判定；完整隊伍傷害落在幻朱雀 72m 固定範圍內時仍須先完成普攻檢核。回補候選依副本專用 `reference_version` 與量測的 `ability_ids` 選取，避免全域 calculation version 升級造成無關場次重查，亦避免舊 ability 7-only 快取漏掉遠程物理職業。
