...
9. 極澤蓮尼亞的 `required_enemy_damage_min`／`required_enemy_damage_max` 是完整隊伍角色列不完整時的固定總傷害替代證據：只能使用既有快取或 FFLogs Target Damage 的敵方承傷，比對同一個 `92,086,132–92,086,332` 範圍。它不能推廣到幻朱雀或其他多目標副本；這些副本的敵方承傷可能包含不同目標，仍只能使用設定明示的角色全隊傷害規則或生命池量測。
