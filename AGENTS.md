...
8. `data_integrity.calculation_version` 只有與 `scripts/fight_integrity.py` 的目前版本相同，才可能作為公開資料的 `valid`／`not_applicable` 結論；較舊版本一律 fail-closed，並由回補器先把已保存的最小量測資料植入不進 Git 的快取後離線重判。`.github/workflows/update_rankings.yml` 必須在推送前比對該輪起始的完整性規則檔指紋；若遠端已變更，舊 runner 必須停止而非 rebase 後回寫過期資料。
