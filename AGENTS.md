...
11. 絕伊甸的 `maximum_enemy_damage=151,500,000` 與既有 `maximum_full_party_damage` 是互補的單向上限：玩家列合計會漏掉 Limit Break 等來源，僅以它判斷會讓 `c6KvhDyzCNXJbRMZ` fight 7 類「角色合計 150.559m、實際敵方承傷 152.652m」資料漏網。只有已保存或新查得的 FFLogs Target Damage 可以套用前者；超過任一上限都標為 `suspected` 並隱藏，未超過則不得由此判定正常。
