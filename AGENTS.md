...
7. `config/fight_integrity_known_enemy_hp.json` 與 `scripts/fight_integrity_known_capacity.py` 是比歷史預篩更優先、可獨立撤除的固定規則。極澤蓮尼亞要求完整繁中隊伍的 `players[].total_damage` 總和落在 `92,086,132–92,086,332`；幻朱雀依站務暫時性收錄決定，要求落在 `71,280,000–72,720,000`（72m ±1%）。兩者低於或高於範圍一律標記異常並隱藏，範圍內且沒有 `Attack` 標記才可直接寫入 `valid`，無須讀取 FFLogs API。當 `players[]` 未完整涵蓋隊伍時，兩者必須改以既有快取或 FFLogs Target Damage 的敵方承傷比較相同範圍；不可因玩家列缺人而跳過固定總傷害規則。幻朱雀的固定生命池 `127,613,543` 仍保留作為可追溯量測資訊，但固定範圍優先於下限倍率。絕伊甸、絕巴哈姆特、M1、M3、M4 的完整隊伍總傷害高於 `151,500,000`、`13,230,230`、`75,870,000`、`96,523,000`、`114,526,000` 時，一律標示為 `suspected` 並隱藏。這些是只有超標才成立的獨立硬上限，不能將未超標資料寫成 `valid`；它們在多階段副本的生命池 `not_applicable` 之前執行，避免 243m 類異常藉此例外重新出現在公開資料。切點後缺少結果、`unverifiable` 或任何非 `valid`／`not_applicable` 狀態均採 fail-closed，不能出現在排行榜、個人成績、隊伍榜或近期動態。`backfill_fight_integrity.py --offline-only` 不建立 OAuth、不呼叫 API，只用既有快取、固定規則和歷史基準；不能離線確認時保留 `unverifiable` 並隱藏。
...
9. 極澤蓮尼亞與幻朱雀的 `required_enemy_damage_min`／`required_enemy_damage_max` 是完整隊伍角色列不完整時的固定總傷害替代證據：只能使用既有快取或 FFLogs Target Damage 的敵方承傷，分別比對 `92,086,132–92,086,332` 與 `71,280,000–72,720,000` 範圍。這是副本個別明示的例外，不得推廣到其他多目標副本；未設定此範圍者仍只能使用角色全隊傷害規則或生命池量測。
